...
const LABEL = { cobrado: 'Cobrado', parcial: 'Parcial', pendiente: 'Pendiente' }
...
function MontoCell({ g }) {
  ...
  return (
    <div className="text-right">
      ...
      {g.tipoCambioCobro && <div className="text-xs text-blue-500">TC cobro ${g.tipoCambioCobro.toLocaleString('es-AR')}</div>}
    </div>
  )
}

function EstadoCell({ ingreso, onActualizarEstado }) {
  ...
  function aplicar(estado) {
    ...
    onActualizarEstado(ingreso.id, estado, null)
    setOpen(false)
  }
  ...
  function confirmarParcial() {
    ...
    if (!val || val <= 0) return
    onActualizarEstado(ingreso.id, 'parcial', val)
    setOpen(false)
  ...
  return (
    <div className="relative">
      <button onClick={() => setOpen(v => v ? false : 'menu')}
        className={`px-2 py-1 rounded-full text-xs font-semibold ${BADGE[ingreso.estado]} cursor-pointer hover:opacity-80 transition whitespace-nowrap`}>
        {LABEL[ingreso.estado]}
        {ingreso.estado === 'parcial' && ingreso.montoParcial ? ` ${fmtARS(ingreso.montoParcial)}` : ''}
      </button>
      {open === 'menu' && (
        <div className="absolute z-20 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg min-w-36 right-0">
          {['cobrado', 'parcial', 'pendiente'].map(e => (
      ...
      {open === 'parcial' && (
        <div className="absolute z-20 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg p-3 right-0 min-w-44">
          <p className="text-xs text-gray-600 mb-2 font-medium">Monto cobrado</p>
          <input type="number" autoFocus value={montoParcialInput}
          ...
          <div className="flex gap-2">
            ...
            <button onClick={confirmarParcial} className="flex-1 text-xs bg-blue-700 text-white rounded py-1 hover:bg-blue-800">OK</button>
          </div>
        </div>
      )}
    </div>
...
export default function IngresoTable({ ingresos, onEliminar, onEditar, onActualizarEstado }) {
  if (ingresos.length === 0) {
    return (
      <div className="text-center py-16 text-gray-400">
        <p className="text-4xl mb-3">💰</p>
        <p className="text-sm">No hay ingresos registrados.</p>
        <p className="text-xs mt-1">Presioná "+ Nuevo ingreso" para comenzar.</p>
      </div>
    )
  }
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-100 text-gray-600 text-xs uppercase tracking-wide">
            <tr>
              ...
              <th className="px-4 py-3 text-center">F. Carga</th>
              <th className="px-4 py-3 text-center">Fecha cobro</th>
              <th className="px-4 py-3 text-left">Forma de cobro</th>
          ...
          <tbody className="divide-y divide-gray-100">
            {ingresos.map(g => (
              <tr key={g.id} className="hover:bg-gray-50 transition">
                ...
                <td className="px-4 py-3 text-center text-gray-400 text-xs">{fmtFechaCarga(g.created_at)}</td>
                <td className="px-4 py-3 text-center text-gray-600">{fmtFecha(g.fecha)}</td>
                <td className="px-4 py-3 text-gray-600">{g.formaCobro}</td>
                <td className="px-4 py-3 text-center">
                  <EstadoCell ingreso={g} onActualizarEstado={onActualizarEstado} />
                </td>
                <td className="px-4 py-3 text-right">
                  <button onClick={() => onEditar(g)} className="text-gray-400 hover:text-blue-600 mr-2 transition">✏️</button>
        ...
        {ingresos.map(g => (
          <div key={g.id} className="bg-white rounded-xl shadow p-4 space-y-2">
            ...
            <div className="flex items-center justify-between text-xs text-gray-500">
              <span>Carga: {fmtFechaCarga(g.created_at)}</span>
              <span>{fmtFecha(g.fecha)}</span>
              <span>{g.formaCobro}</span>
            </div>
          </div>
        ))}
      </div>
